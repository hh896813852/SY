package com.edusoho.yunketang.widget.slide;
...
public class SlideView extends RelativeLayout implements SeekBar.OnSeekBarChangeListener, SeekBar.OnTouchListener {
    ...
    private boolean                         slideTextCenter;
    ...
    void init(AttributeSet attrs, int defStyle) {
        inflate(getContext(), R.layout.sb_view, this);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            setBackground(ContextCompat.getDrawable(getContext(), R.drawable.sb_view_bg));
        } else {
            setBackgroundDrawable(ContextCompat.getDrawable(getContext(), R.drawable.sb_view_bg));
        }
        ...
        int strokeColor;
        float slideTextSize = DensityUtil.dip2px(getContext(), 16);
        String slideText;
        ...
        try {
            ...
            strokeColor = a.getColor(R.styleable.SlideView_sv_strokeColor, ContextCompat.
                    getColor(getContext(), R.color.bg_primary));
            ...
            slideTextCenter = a.getBoolean(R.styleable.SlideView_sv_slideTextCenter, false);
            ...
            int buttonImageId = a.getResourceId(R.styleable.SlideView_sv_buttonImage, R.drawable.sb_right);
            ...
            if (a.hasValue(R.styleable.SlideView_sv_strokeColor)) {
                AppUtils.setDrawableStroke(slideBackground, strokeColor);
            }
            if (reverseSlide) {
    ...
    public void setButtonBackgroundColor(ColorStateList color) {
        ...
        AppUtils.setDrawableColor(buttonBackground, color.getDefaultColor());
    }


    public void setSlideBackgroundColor(ColorStateList color) {
        slideBackgroundColor = color;
        AppUtils.setDrawableColor(slideBackground, color.getDefaultColor());
    }
    ...
    public void setEnabled(boolean enabled) {
        ...
        AppUtils.setDrawableColor(buttonBackground, buttonBackgroundColor.getColorForState(
                enabled ? new int[]{android.R.attr.state_enabled} : new int[]{-android.R.attr.state_enabled}
                , ContextCompat.getColor(getContext(), android.R.color.white)));
        AppUtils.setDrawableColor(slideBackground, slideBackgroundColor.getColorForState(
                enabled ? new int[]{android.R.attr.state_enabled} : new int[]{-android.R.attr.state_enabled}
                , ContextCompat.getColor(getContext(), android.R.color.white)));
    }
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        Log.d("flag--", "onProgressChanged: " + progress);
        if (mOnSeekBarChangeListener != null) {
            mOnSeekBarChangeListener.onProgressChanged(seekBar, progress, fromUser);
        }
        if (animateSlideText) {
            float percent = this.slider.getMax() * 1.0f;
            if (slideTextCenter) {
                percent = percent / 2;
            }
            slideTextView.setAlpha(1 - (progress / percent));
        }
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        if (mOnSeekBarTouchListener != null) {
            mOnSeekBarTouchListener.onTouch(v, event);
        }
        return false;
    }
    ...
    public void setSeekBarTouchListener(OnTouchListener onSeekBarTouchListener) {
        mOnSeekBarTouchListener = onSeekBarTouchListener;
    }
